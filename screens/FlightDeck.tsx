...
const getMinimumEffort = (growth: GrowthState, settings: any): number => {
  if (settings.maintenanceMode) return 1;
  if (settings.stateOverride === 'FORCE_RECOVERY') return 1;
  if (settings.stateOverride === 'FORCE_PUSH') return Math.max(60, Math.floor(growth.peakWeeklyAverageEffort * 1.1));
...
export const FlightDeck: React.FC = () => {
  const { daily, capacity, growth, settings, createTask, generateGroundTask, completeTask, updateSettings } = useWingsStore();
  ...
  const minEffort = getMinimumEffort(growth, settings);
